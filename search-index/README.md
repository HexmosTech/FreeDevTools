...
### How to Index Search and Deploy Search Index 

This Procedure help you to do search index for all categories and index see also.

This Involves Updating Existing db's.

So, Avoid DB conflicts by mentioning others not to update DBs.

Step 1: Check out main branch and pull latest changes code.
```shell
git checkout main
git pull origin main
```
Step 2: Pull latest DBs from B2
```shell
...
This will generated all categories JSON files in `search-index/output/`

For validation
Check Diff of JSON files.

Step 4: Make ready search
```shell
make ready-search
```
This will pull docker image and start meilisearch container.

#### Indexing and Updating See also in DB

Step 1: Add see also column
```shell
make add-see-also-column
```

Step 2: Index see also
```shell
make index-see-also items=9
```
> Note: Items=9 is for 9 see also items
> For Faster Indexing use `make index-see-also-fast items=9`

Step 3: Update DBs to B2

```shell
make update-db-to-b2 <DB_PATH>
```


#### Indexing and Deploying Search Index

Pre-requisite: Make sure meilisearch is running
In SSH Config you have `master-do` 

Step 1: Build new binary to avoid any issues

```shell
cd search-index && make build
```
Step 2: Transfer JSON files to remote server
```shell
make sync-search-index
```
