...
func generateManPagesData(ctx context.Context) ([]ManPageData, error) {
	...
	query := `
		SELECT hash_id, title, filename, main_category, sub_category, slug
		FROM man_pages
	...
	for rows.Next() {
		...
		if err := rows.Scan(&hashID, &title, &filename, &mainCategory, &subCategory, &slug); err != nil {
