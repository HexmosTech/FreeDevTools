...
3.  **CheckB3SumAvailability**: Verifies if `b3sum` is installed in PATH.
4.  **RunInit** (in `core/init.go`): Initializes `rclone` by installing it (if missing) and creating the configuration file.
5.  **BootstrapSystem**: Performs initial checks for database discovery and synchronization.
6.  **checkDBDiscoveryAndSync**: (Internal) Orchestrates local and remote database discovery.
7.  **getLocalDBs** (in `core/local.go`): Lists local database files.
8.  **LsfRclone**: Lists all files recursively using `rclone lsf -R` to retrieve both databases and locks efficiently.
9.  **checkFileChanged**: Checks if a specific file has changed between local and remote using `rclone check`.
10. **SyncDatabase** / **DownloadDatabase**: (Moved to `core/download.go`) Downloads a single database.
11. **UploadDatabase** (in `core/upload.go`): Uploads a single database from local to remote. **Returns** the uploaded `model.Metadata` object for anchor persistence.
12. **LockDatabase** (in `core/handleFiles.go`): Creates a lock file (`.lock`) on the remote.
13. **UnlockDatabase** (in `core/handleFiles.go`): Removes a lock file from the remote.
14. **FetchLocks**: Retrieves and parses all active lock files from the remote.
15. **RcloneCopy**: Wrapper for `rclone copy` commands with logging and progress tracking.
16. **RcloneSync**: Wrapper for `rclone sync`.
