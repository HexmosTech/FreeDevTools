...
func NewDB(dbPath string) (*DB, error) {
	...
	connStr := dbPath + db_config.SVGIconsDBConfig
...
func (db *DB) GetIconsByCluster(cluster string, categoryName *string, limit, offset int) ([]IconWithMetadata, error) {
	...
	// println(cluster)
	hashName := HashURLToKey(cluster)
	// println(hashName)
	query := `SELECT id, cluster, name, base64,
	...
	for rows.Next() {
		...
		if categoryName != nil {
			...
			url := fmt.Sprintf("/freedevtools/svg_icons/%s/%s/", *categoryName, row.Name)
			icon.URL = &url
		}
	...
	if transform {
		...
		for rows.Next() {
			...
			transformed := ClusterTransformed{
				...
				Name:         row.Name,
				Description:  "",
				Icon:         fmt.Sprintf("/freedevtools/svg_icons/%s/", row.Name),
				IconCount:    row.Count,
				URL:          fmt.Sprintf("/freedevtools/svg_icons/%s/", row.Name),
				Keywords:     []string{},
				Features:     []string{},
...
func (db *DB) GetClusterByName(hashName string) (*Cluster, error) {
...
func (db *DB) GetIconByCategoryAndName(category, iconName string) (*Icon, error) {
	...
	// The database stores URLs in format: /{source_folder}/{icon_name} (no trailing slash, no prefix)
	url := fmt.Sprintf("/%s/%s", cluster.SourceFolder, iconName)
	...
	if err != nil {
		if err == sql.ErrNoRows {
			fmt.Printf("Fallback query executed for SVG: %s/%s\n", cluster.SourceFolder, iconName)
...
func (db *DB) GetIconUpdatedAt(sourceFolder string, iconName string) (string, error) {
	url := fmt.Sprintf("/%s/%s", sourceFolder, iconName)
	urlHash := HashURLToKey(url)
