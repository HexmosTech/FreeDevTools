(function () {
    ...
    const renderEmpty = () => {
        grid.innerHTML = '<div class="col-span-3 text-center text-slate-600 dark:text-slate-400 p-6">No related content found</div>';
    };
    ...
    const renderItems = (items) => {
        ...
        grid.innerHTML = items.map(item => {
            ...
            if (item.code) {
                ...
                iconHtml = '<div class="flex-shrink-0 text-slate-600 dark:text-slate-400 group-hover:text-fdt-yellow-dark dark:group-hover:text-yellow-500 group-hover:scale-105 transition-all duration-200 w-12 h-12 flex items-center justify-center">' +
                    item.icon +
                    '</div>';
            }

            return '<a href="' + item.link + '"' +
                '   class="see-also-card flex flex-col items-center gap-4 p-6 rounded-xl border border-border bg-background hover:border-fdt-yellow-dark dark:hover:border-yellow-700 hover:shadow transition-all duration-200 group cursor-pointer">' +
                iconHtml +
                '<span class="text-base font-semibold text-center text-slate-700 dark:text-slate-300 group-hover:text-fdt-yellow-dark dark:group-hover:text-yellow-500 transition-colors duration-200">' + item.text + '</span>' +
