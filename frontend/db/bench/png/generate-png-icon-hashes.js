...
const config = {
  ...
  jsonPath: path.join(__dirname, 'png_urls.json')
};
...
function migrateClusterTable(db) {
  console.log('🔄 Migrating cluster table...');
  ...
  const insertStmt = db.prepare(`
    INSERT INTO cluster_new (
      id, hash_name, name, source_folder, path, count, keywords, tags,
      title, description, practical_application, alternative_terms,
      about, why_choose_us
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  console.log('✅ Cluster table migrated successfully.');
}
...
async function main() {
  ...
  // Always migrate cluster table
  migrateClusterTable(db);
