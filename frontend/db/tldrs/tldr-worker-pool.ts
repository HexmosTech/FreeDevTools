/**
 * Worker pool manager for TLDR SQLite queries using bun:sqlite
...
async function initWorkers(): Promise<void> {
  ...
  console.log(
    `[TLDR_DB] Initializing worker pool with ${WORKER_COUNT} workers...`
  );
  ...
  initPromise = new Promise((resolve, reject) => {
    ...
    const relativeWorkerPath = path.join(__dirname, 'tldr-worker');
    ...
    for (let i = 0; i < WORKER_COUNT; i++) {
      ...
      worker.on('message', (msg) => {
        if (msg.ready) {
          ...
          if (initializedCount === WORKER_COUNT) {
            ...
            console.log(
              `[TLDR_DB] Worker pool initialized in ${initEndTime - initStartTime}ms`
            );
            initPromise = null;
            resolve();
          }
      ...
      worker.on('error', (err) => {
        console.error(`[TLDR_DB] Worker ${i} error:`, err);
        initPromise = null;
        reject(err);
      });
      ...
      worker.on('exit', (code) => {
        if (code !== 0) {
          console.error(`[TLDR_DB] Worker ${i} exited with code ${code}`);
        }
      });
...
void initWorkers().catch((err) => {
  console.error('[TLDR_DB] Failed to warm worker pool:', err);
});
