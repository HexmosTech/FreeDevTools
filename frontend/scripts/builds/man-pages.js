...
function excludeUnchangedSections() {
  logStep('Excluding unchanged sections from build', 'Building only man-pages section...');

  const changedSections = ['man-pages']; // Only build man-pages
  logInfo(`Building strategy: ${changedSections.join(' ')}`);

  logInfo('🎯 Selective build mode - man-pages only');
  ...
  for (const dir of dirs) {
    ...
    // Only include man-pages, exclude everything else (including already excluded directories)
...
function buildProject() {
  logStep('Building project', 'Running Astro build for man-pages section only...');
...
function showBuildInfo() {
  log('\n📦 man-pages Build Script', 'magenta');
  log('==================', 'magenta');
  log('This script will build only the man-pages section by excluding all other page directories.', 'white');
...
async function main() {
  try {
    ...
    logSuccess('\n🎉 man-pages build completed successfully!');
