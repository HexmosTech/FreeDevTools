...
function excludeAllDirectories() {
  logStep(
    'Excluding all directories from build',
    'Building only index.astro...'
  );

  logInfo('🎯 Selective build mode - index.astro only');
...
function buildProject() {
  logStep('Building project', 'Running Astro build for index.astro only...');
...
function showBuildInfo() {
  log('\n📦 Index Build Script', 'magenta');
  log('====================', 'magenta');
  log(
    'This script will build only index.astro by excluding all page directories.',
    'white'
  );
  log(
...
async function main() {
  try {
    ...
    // Step 1: Exclude all directories
