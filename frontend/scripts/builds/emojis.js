...
function excludeUnchangedSections() {
    logStep('Excluding unchanged sections from build', 'Building only emojis section...');

    const changedSections = ['emojis']; // Only build emojis
    logInfo(`Building strategy: ${changedSections.join(' ')}`);

    logInfo('🎯 Selective build mode - emojis only');
    ...
    for (const dir of dirs) {
        ...
        // Only include emojis, exclude everything else (including already excluded directories)
...
function buildProject() {
    logStep('Building project', 'Running Astro build for emojis section only...');
...
function showBuildInfo() {
    log('\n📦 emojis Build Script', 'magenta');
    log('==================', 'magenta');
    log('This script will build only the emojis section by excluding all other page directories.', 'white');
...
async function main() {
    try {
        ...
        logSuccess('\n🎉 emojis build completed successfully!');
