...
function excludeUnchangedSections() {
    logStep('Excluding unchanged sections from build', 'Building only cheatsheets section...');

    const changedSections = ['c']; // Only build cheatsheets (c directory)
    logInfo(`Building strategy: ${changedSections.join(' ')}`);

    logInfo('🎯 Selective build mode - cheatsheets only');
    ...
    for (const dir of dirs) {
        ...
        // Only include cheatsheets (c), exclude everything else (including already excluded directories)
...
function buildProject() {
    logStep('Building project', 'Running Astro build for cheatsheets section only...');
...
function showBuildInfo() {
    log('\n📦 cheatsheets Build Script', 'magenta');
    log('==================', 'magenta');
    log('This script will build only the cheatsheets section (c/) by excluding all other page directories.', 'white');
...
async function main() {
    try {
        ...
        logSuccess('\n🎉 cheatsheets build completed successfully!');
