...
def generate_unique_slug(
    conn: sqlite3.Connection, base_slug: str, main_category: str, sub_category: str
) -> str:
...
def process_failed_files(conn: sqlite3.Connection) -> int:
    ...
    all_md_files = []
    main_categories = [
        d for d in MD_DIR.iterdir() if d.is_dir() and not d.name.startswith(".")
    ]

    for main_cat_dir in main_categories:
        sub_categories = [
            d
            for d in main_cat_dir.iterdir()
        ...
        try:
            ...
            if result:
                ...
                if base_slug != unique_slug:
                    logger.info(
                        f"✅ Inserted {filename} with unique slug: {unique_slug}"
                    )
                else:
                ...
                if i % 100 == 0:
                    logger.info(
                        f"  📄 Processed {i}/{len(missing_files)} missing files, inserted {inserted}"
                    )
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_man_pages_main_category ON man_pages(main_category);"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_man_pages_sub_category ON man_pages(sub_category);"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_man_pages_category_sub ON man_pages(main_category, sub_category);"
    )
    cur.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_man_pages_filename ON man_pages(filename);"
    )
    cur.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_man_pages_slug ON man_pages(main_category, sub_category, slug);"
    )
...
def parse_markdown_frontmatter(md_file: Path) -> Tuple[Dict[str, Any], str]:
    ...
    try:
        ...
        try:
            ...
            fallback_frontmatter = {"title": md_file.stem, "content": {}}
    ...
    for i, line in enumerate(lines):
        ...
        if ":" in line and not in_multiline_value:
            ...
            if value.startswith('"') and (
                not value.endswith('"') or value.count('"') == 1
            ):
                ...
                value.startswith("<") or "catmandu" in value.lower() or "pre>" in value
            ):
...
def load_markdown_files(conn: sqlite3.Connection) -> Tuple[int, int]:
    ...
    slug_tracker = (
        {}
    )  # Track slugs per category/subcategory to handle duplicates in current batch
    ...
    for result in all_results:
        ...
        try:
            ...
            if base_slug in slug_tracker[category_key]:
                ...
                logger.warning(
                    f"Duplicate slug detected: {base_slug} → {final_slug} for {filename}"
                )
            ...
            if inserted % 10000 == 0:
                ...
                logger.info(
                    f"Progress: {inserted:,} / {len(all_results):,} records inserted"
                )
    ...
    conn.commit()
    logger.info(
        f"✅ Successfully inserted {inserted:,} out of {len(all_results):,} processed files"
    )
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if inserted < files * 0.95:  # If less than 95% success rate, check for failures
            ...
            if failed_inserted > 0:
                logger.info(
                    f"Additionally inserted {failed_inserted} previously failed files"
                )
        else:
