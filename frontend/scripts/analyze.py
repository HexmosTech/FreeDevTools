#!/usr/bin/env python3
"""
Log Analysis Script for Emoji Database Queries
...
def analyze_log_file(log_file_path):
    ...
    with open(log_file_path, 'r') as f:
        for line in f:
            # Parse new format: [EMOJI_DB] Emoji|Apple|Discord <queryname> <time>
            # Format: [EMOJI_DB] Emoji getEmojiImages 123.456ms
            match = re.search(r'\[EMOJI_DB\]\s+(Emoji|Apple|Discord)\s+(\w+)\s+([\d.]+(?:ms|s))', line)
            if match:
                vendor, query_name, time_str = match.groups()
                timing = parse_timing(time_str)
                if timing:
                    # Create unique key: vendor_queryname
                    key = f"{vendor}_{query_name}"
                    query_times[key].append(timing)
                continue
            
            # Parse paginated emojis query: "Paginated emojis query took <time>"
            if "Paginated emojis query took" in line:
                match = re.search(r'([\d.]+(?:ms|s))', line)
                if match:
                    timing = parse_timing(match.group(1))
                    if timing:
                        query_times['Paginated emojis query'].append(timing)
                continue
            
            # Parse "Processed X images in <time>" - keep for reference but don't count as main query
            if "Processed" in line and "images in" in line:
                # Keep this for reference but don't add to main stats
                continue
...
def parse_query_key(query_key):
    ...
    # Handle special case: "Paginated emojis query"
    if query_key == "Paginated emojis query":
        return ("Emoji", "Paginated emojis query")
    
    # Split on first underscore: "Emoji_getEmojiBySlug" -> ("Emoji", "getEmojiBySlug")
    if '_' in query_key:
...
def print_report(query_times):
    ...
    print("=" * 130)
    print("EMOJI DATABASE QUERY PERFORMANCE ANALYSIS")
    print("=" * 130)
    print()
