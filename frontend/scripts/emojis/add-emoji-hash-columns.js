...
const config = {
  dbPath: dbArg
    ...
    : path.join(__dirname, 'emoji-db-v1.db'),
};
...
function populateHashColumns(db) {
  ...
  console.log(
    `✅ Updated ${updates.length.toLocaleString()} rows with hash values`
  );
}
...
function verifyHashes(db) {
  ...
  console.log(
    `   ${rowsWithHashes.count.toLocaleString()} / ${totalRows.count.toLocaleString()} rows have hash values`
  );
  ...
  if (rowsWithHashes.count === totalRows.count) {
    ...
    console.warn(
      `⚠️  ${totalRows.count - rowsWithHashes.count} rows missing hash values`
    );
  }
}
