...
DB_PATH = Path(__file__).parent.parent.parent / "db" / "png_icons" / "png-icons-db.db"
...
def load_cluster_png_data() -> dict:
    """Load cluster_png.json metadata."""
    if not CLUSTER_PNG_PATH.exists():
        print(
            f"⚠ cluster_png.json not found at {CLUSTER_PNG_PATH}, using empty metadata"
        )
        return {}

    try:
        with open(CLUSTER_PNG_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data.get("clusters", {})
    except Exception as e:
        print(f"⚠ Failed to read cluster_png.json: {e}, using empty metadata")
        return {}
...
def load_json_files(conn: sqlite3.Connection) -> tuple[int, int]:
    ...
    cluster_metadata = load_cluster_png_data()
    cur = conn.cursor()
    inserted = 0
...
def populate_cluster_and_overview(conn: sqlite3.Connection) -> None:
    """Populate cluster and overview tables from icon table and cluster_png.json."""
    cur = conn.cursor()
    cluster_metadata = load_cluster_png_data()
    ...
    for cluster_name in clusters:
        ...
        # Get metadata from cluster_png.json
