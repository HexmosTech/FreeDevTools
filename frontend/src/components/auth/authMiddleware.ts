...
export const authMiddleware = defineMiddleware(async (context, next) => {
  ...
  console.log(
    `[Auth Middleware] ENABLE_SIGNIN env value: "${process.env.ENABLE_SIGNIN}", enabled: ${enableSignin}, Path: ${pathname}`
  );
  ...
  if (enableSignin) {
    ...
    const isStaticAsset =
      ...
      pathname.match(
        /\.(js|css|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|eot|json|xml|txt|map)$/i
      ) ||
    ...
    if (!isStaticAsset) {
      ...
      if (!hasDataParam) {
        ...
        if (!jwt) {
          ...
          console.log(
            `[Auth Middleware] No JWT found, redirecting to: ${signinUrl}`
          );
