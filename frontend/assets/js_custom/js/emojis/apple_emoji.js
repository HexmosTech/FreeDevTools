(function () {
    if (window._appleEmojiInitialized) return;
    window._appleEmojiInitialized = true;
    ...
    function initCopyButton() {
        const btn = document.getElementById('copy-apple-emoji-btn');
    ...
    // Initialize all Apple emoji functionality
    function initializeAppleEmoji() {
        initCopyButton();
        initEvolutionImages();
    }

    // Try immediately
    initializeAppleEmoji();

    // Also try on DOMContentLoaded in case script runs before button exists
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeAppleEmoji);
    }
})();
